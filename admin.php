...
// INISIALISASI VARIABEL dan PROSES CRUD hanya jika sudah login
if (isset($_SESSION['admin_id'])) {
    ...
    if($result_produk) { while($row = mysqli_fetch_assoc($result_produk)) { $recent_produk[] = $row; } }

    $recent_berita = [];
    $result_berita = mysqli_query($koneksi, "SELECT id_berita, judul, gambar, tanggal FROM berita ORDER BY id_berita DESC LIMIT 5");
    if($result_berita) { while($row = mysqli_fetch_assoc($result_berita)) { $recent_berita[] = $row; } }

    // PROSES CRUD BERITA
    if (isset($_POST['tambah_berita'])) {
        $judul = bersihkan_input($_POST['judul']); $isi = bersihkan_input($_POST['isi']); $gambar = '';
        ...
        if (empty($error_upload)) {
            $query = "INSERT INTO berita (judul, isi, gambar) VALUES (?, ?, ?)"; $stmt = mysqli_prepare($koneksi, $query);
    ...
    if (isset($_GET['hapus_berita'])) {
        $id = bersihkan_input($_GET['hapus_berita']); $gambar = ambil_satu_kolom('berita', 'gambar', 'id_berita', $id, $koneksi);
        if ($gambar && file_exists("uploads/" . $gambar)) { unlink("uploads/" . $gambar); }
        $query = "DELETE FROM berita WHERE id_berita = ?"; $stmt = mysqli_prepare($koneksi, $query);
        mysqli_stmt_bind_param($stmt, "i", $id); mysqli_stmt_execute($stmt); mysqli_stmt_close($stmt);
        header("Location: admin.php?status=berita_dihapus#berita-tab"); exit();
    }
    
    // PROSES CRUD PRODUK
    if (isset($_POST['tambah_produk'])) {
        $nama_produk = bersihkan_input($_POST['nama_produk']); $deskripsi = bersihkan_input($_POST['deskripsi']); $harga = (int)bersihkan_input($_POST['harga']); $stok = (int)bersihkan_input($_POST['stok']); $gambar = '';
        if (isset($_FILES['gambar_produk']) && $_FILES['gambar_produk']['error'] == 0) {
            $target_dir = "uploads/produk/"; if (!is_dir($target_dir)) mkdir($target_dir, 0777, true);
        ...
        if (empty($error_upload)) {
            $query = "INSERT INTO produk (nama_produk, deskripsi, harga, stok, gambar) VALUES (?, ?, ?, ?, ?)"; $stmt = mysqli_prepare($koneksi, $query);
    ...
    if (isset($_POST['update_produk'])) {
        $id_produk = (int)bersihkan_input($_POST['id_produk']); $nama_produk = bersihkan_input($_POST['nama_produk']); $deskripsi = bersihkan_input($_POST['deskripsi']); $harga = (int)bersihkan_input($_POST['harga']); $stok = (int)bersihkan_input($_POST['stok']); $gambar_lama = bersihkan_input($_POST['gambar_lama']); $gambar = $gambar_lama;
        if (isset($_FILES['gambar_produk']) && $_FILES['gambar_produk']['error'] == 0) {
        ...
        if (empty($error_upload)) {
            $query = "UPDATE produk SET nama_produk=?, deskripsi=?, harga=?, stok=?, gambar=? WHERE id_produk=?"; $stmt = mysqli_prepare($koneksi, $query);
    ...
    if (isset($_GET['hapus_produk'])) {
        $id = bersihkan_input($_GET['hapus_produk']); $gambar = ambil_satu_kolom('produk', 'gambar', 'id_produk', $id, $koneksi);
        if ($gambar && file_exists("uploads/produk/" . $gambar)) { unlink("uploads/produk/" . $gambar); }
        $query = "DELETE FROM produk WHERE id_produk = ?"; $stmt = mysqli_prepare($koneksi, $query);
        mysqli_stmt_bind_param($stmt, "i", $id); mysqli_stmt_execute($stmt); mysqli_stmt_close($stmt);
        header("Location: admin.php?status=produk_dihapus#produk-tab"); exit();
    }
    
    // AMBIL DATA UNTUK FORM EDIT
    $produk_diedit = null;
    if (isset($_GET['edit_produk'])) {
        $id_produk_edit = bersihkan_input($_GET['edit_produk']); $query = "SELECT * FROM produk WHERE id_produk = ?";
    ...
    if (isset($_GET['edit_berita'])) {
        $id_berita_edit = bersihkan_input($_GET['edit_berita']); $query = "SELECT * FROM berita WHERE id_berita = ?";
...
<head>
    ...
    <title><?= isset($_SESSION['admin_id']) ? 'Admin Panel' : 'Admin Login' ?> - Petani Maju</title>
    ...
    <style>
        ...
        /* --- STYLE KETIKA SUDAH LOGIN (PANEL ADMIN) --- */
        body.admin-page {
            background-color: #f4f7f6; padding-top: 20px; font-family: 'Segoe UI', Tahoma, Verdana, sans-serif;
        }
        ...
        /* --- STYLE BARU UNTUK HALAMAN LOGIN --- */
        body.login-page {
            background-image: url('https://images.unsplash.com/photo-1492496913980-501348b61469?q=80&w=1887&auto=format&fit=crop');
        ...
        .login-logo { font-size: 4.5rem; color: var(--primary-light); text-shadow: 0 0 15px rgba(76, 175, 80, 0.7); }
        .login-header h2 { font-weight: 600; text-shadow: 1px 1px 3px rgba(0,0,0,0.5); }
        .login-header p { opacity: 0.9; }
        .login-input.form-control {
            background: transparent; border: none; border-bottom: 2px solid rgba(255, 255, 255, 0.5);
            border-radius: 0; color: white; padding: 10px 0;
        }
        .login-input.form-control:focus { box-shadow: none; border-bottom-color: var(--primary-light); }
        .login-input.form-control::placeholder { color: rgba(255, 255, 255, 0.7); }
        .btn-login {
            background: var(--primary-light); border: none; padding: 12px; font-weight: 600;
            border-radius: 50px; transition: all 0.3s ease;
        }
        .btn-login:hover {
            background-color: var(--primary); transform: translateY(-3px); box-shadow: 0 5px 20px rgba(76, 175, 80, 0.4);
        }
    </style>
</head>
<body class="<?= isset($_SESSION['admin_id']) ? 'admin-page' : 'login-page' ?>">

    <?php if(!isset($_SESSION['admin_id'])): ?>
        <!-- ================= HALAMAN LOGIN BARU ================= -->
        <div class="login-glass-card">
            <div class="login-header">
                <div class="login-logo"><i class="fas fa-tractor"></i></div>
                <h2 class="mt-2">Petani Maju</h2>
                <p>Silakan masuk untuk melanjutkan</p>
            </div>
            
            <?php if($error): ?>
                <div class="alert alert-danger py-2 small text-center bg-danger bg-opacity-50 border-0 text-white"><?= htmlspecialchars($error) ?></div>
            <?php endif; ?>
            ...
            <form method="POST">
                ...
                <div class="d-grid mt-4">
                    <button type="submit" name="login" class="btn btn-login text-white"><i class="fas fa-sign-in-alt me-2"></i> Masuk</button>
                </div>
            </form>
        </div>
    ...
    <?php else: ?>
        ...
        <div class="admin-container">
            ...
            $tab_produk_aktif = isset($_GET['edit_produk']) || strpos($_SERVER['REQUEST_URI'], 'produk') !== false;
            $tab_berita_aktif = isset($_GET['edit_berita']) || strpos($_SERVER['REQUEST_URI'], 'berita') !== false;
            $dashboard_active = !($tab_produk_aktif || $tab_berita_aktif) ? 'active' : '';
            ?>
            ...
            <ul class="nav nav-tabs" id="adminTab" role="tablist">
                <li class="nav-item" role="presentation"><button class="nav-link <?= $dashboard_active ?>" data-bs-toggle="tab" data-bs-target="#dashboard-tab" type="button">Dashboard</button></li>
                <li class="nav-item" role="presentation"><button class="nav-link <?= $tab_produk_aktif ? 'active' : '' ?>" data-bs-toggle="tab" data-bs-target="#produk-tab" type="button">Manajemen Produk</button></li>
                <li class="nav-item" role="presentation"><button class="nav-link <?= $tab_berita_aktif ? 'active' : '' ?>" data-bs-toggle="tab" data-bs-target="#berita-tab" type="button">Manajemen Berita</button></li>
            </ul>
            ...
            <div class="tab-content" id="adminTabContent">
                            ...
                            <ul class="list-group list-group-flush">
                                <?php if(empty($recent_produk)): ?><li class="list-group-item">Belum ada produk.</li><?php else: ?>
                                    ...
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <div><?php if($item['gambar']): ?><img src="uploads/produk/<?= $item['gambar'] ?>" class="news-img me-2" alt="..."><?php endif; ?><strong><?= htmlspecialchars($item['nama_produk']) ?></strong></div>
                ...
                <!-- ======================= TAB PRODUK ======================= -->
                <div class="tab-pane fade show <?= $tab_produk_aktif ? 'active' : '' ?>" id="produk-tab" role="tabpanel">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <?php if ($produk_diedit): ?><button type="submit" name="update_produk" class="btn btn-primary"><i class="fas fa-save me-2"></i> Simpan</button><a href="admin.php?tab=produk" class="btn btn-secondary">Batal</a><?php else: ?><button type="submit" name="tambah_produk" class="btn btn-success"><i class="fas fa-plus me-2"></i> Tambah</button><?php endif; ?>
                    </form>
                    <hr class="my-5">
                ...
                <!-- ======================= TAB BERITA ======================= -->
                <div class="tab-pane fade show <?= $tab_berita_aktif ? 'active' : '' ?>" id="berita-tab" role="tabpanel">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <?php if ($berita_diedit): ?><button type="submit" name="update_berita" class="btn btn-primary"><i class="fas fa-save me-2"></i> Simpan</button><a href="admin.php?tab=berita" class="btn btn-secondary">Batal</a><?php else: ?><button type="submit" name="tambah_berita" class="btn btn-success"><i class="fas fa-plus me-2"></i> Tambah</button><?php endif; ?>
                    </form>
                    <hr class="my-5">
