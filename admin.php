...
if (!isset($_SESSION['admin'])) {
    ...
    if (isset($_GET['hapus_berita'])) {
        ...
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
        header("Location: admin.php?status=berita_dihapus");
        exit();
    }
...
<!DOCTYPE html>
<html lang="id">
<!-- Head dan style sama persis, jadi saya singkat untuk kejelasan -->
<head>
    <meta charset="UTF-8">
...
<body class="admin-container">
    <?php if(!isset($_SESSION['admin'])): ?>
        <!-- Login Form (Tidak ada perubahan) -->
    ...
    <?php else: ?>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="nama" name="nama" required value="<?= $produk_diedit['nama'] ?? '' ?>" placeholder="e.g., Bibit Jagung Hibrida">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label for="kategori" class="form-label">Kategori</label>
                        <input type="text" class="form-control" id="kategori" name="kategori" required value="<?= $produk_diedit['kategori'] ?? '' ?>" placeholder="e.g., Bibit Unggul">
                    </div>
                </div>
                <div class="row">
                     <div class="col-md-6 mb-3">
                        <label for="harga" class="form-label">Harga</label>
                        <input type="text" class="form-control" id="harga" name="harga" required value="<?= $produk_diedit['harga'] ?? '' ?>" placeholder="e.g., Rp 95.000 /kg">
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        <input class="form-control" type="file" id="gambar_produk" name="gambar_produk" accept="image/*">
                         <?php if ($produk_diedit && $produk_diedit['gambar']): ?>
                            <small class="text-muted">Gambar saat ini: <?= $produk_diedit['gambar'] ?></small>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="mb-3">
                    <label for="deskripsi_produk" class="form-label">Deskripsi</label>
                    <textarea class="form-control" id="deskripsi_produk" name="deskripsi" rows="4" required placeholder="Deskripsi singkat produk"><?= $produk_diedit['deskripsi'] ?? '' ?></textarea>
                </div>
                
                <?php if ($produk_diedit): ?>
                    <button type="submit" name="update_produk" class="btn btn-primary"><i class="fas fa-save me-2"></i> Simpan Perubahan</button>
                    <a href="admin.php" class="btn btn-secondary">Batal Edit</a>
                <?php else: ?>
                        ...
                        <tbody>
                            <?php foreach($produk as $item): ?>
                                <tr>
                                    <td>
                                        <?php if($item['gambar']): ?>
                                            <img src="uploads/produk/<?= $item['gambar'] ?>" class="news-img" alt="<?= $item['nama'] ?>">
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <a href="?edit_produk=<?= $item['id_produk'] ?>" class="btn btn-warning btn-sm btn-action"><i class="fas fa-edit"></i> Edit</a>
        ...
        <!-- Panel Berita (Tidak ada perubahan) -->
        <div class="admin-panel">
